...
+ (void)startEngine
{
    ...
    //对应js中的defineClass，为OC的类定义和覆盖方法
...
+ (JSValue *)evaluateScript:(NSString *)script withSourceURL:(NSURL *)resourceURL
{
    ...
    //对JS代码进行预处理,将类似于UIView.alloc()的代码替换为UIView.__c("alloc")
    if (!_regex) {
...
#pragma mark - 将JS中定义的方法加到OC的protocol中
static void addGroupMethodsToProtocol(Protocol* protocol,JSValue *groupMethods,BOOL isInstance)
{
    ...
    for (NSString *jpSelector in groupDic.allKeys) {
        ...
        if (typeEncode) {
            ...
            //根据JSfunction的参数以及返回类型生成typeEncode
...
static NSDictionary *defineClass(NSString *classDeclaration, JSValue *instanceMethods, JSValue *classMethods)
{
    ...
    for (int i = 0; i < 2; i ++) {
        ...
        //如果不是实例方法，就把currCls赋值为metaClass，用来获取类方法
        Class currCls = isInstance ? cls: objc_getMetaClass(className.UTF8String);
        NSDictionary *methodDict = [jsMethods toDictionary];
        for (NSString *jsMethodName in methodDict.allKeys) {
            JSValue *jsMethodArr = [jsMethods valueForProperty:jsMethodName];
            int numberOfArg = [jsMethodArr[0] toInt32];
            //将JS方法转换为OC selector格式
...
static void overrideMethod(Class cls, NSString *selectorName, JSValue *function, BOOL isClassMethod, const char *typeDescription)
{
    ...
    //如果不是插入新方法，typeDescription会是空的，这个时候要从之前存在的方法获取typeDescription
    if (!typeDescription) {
        Method method = class_getInstanceMethod(cls, selector);
        typeDescription = (char *)method_getTypeEncoding(method);
    }
    
    //获取原始的实现，如果是增加新方法就会是NULL
    IMP originalImp = class_respondsToSelector(cls, selector) ? class_getMethodImplementation(cls, selector) : NULL;
    
    //获取_objc_msgForward的IMP，在非__arm64__时需要使用_objc_msgForward_stret
    //具体原因:http://sealiesoftware.com/blog/archive/2008/10/30/objc_explain_objc_msgSend_stret.html
    ...
    //将原始的方法实现定向到_objc_msgForward | _objc_msgForward_stret
    //这样调用该方法是会直接进入forwardInvocation，从而进行转发
    class_replaceMethod(cls, selector, msgForwardIMP, typeDescription);

#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wundeclared-selector"
    //如果该class有实现forwardInvocation，替换掉原有实现，并且增加ORIGforwardInvocation:保存原有的实现
...
#pragma clang diagnostic pop
    ...
    //将要替换的方法原有实现定向到 ORIG原有方法名 的 selector下
